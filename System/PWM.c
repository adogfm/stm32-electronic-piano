...
void PWM_Init_1(void){
	...
	TIM_OCInitStructure.TIM_Pulse = 0;  //CCR  Duty=CCR/(ARR+1), off by default
...
void PWM_SetFreq_1(uint16_t freq){
	...
	TIM_GenerateEvent(TIM2, TIM_EventSource_Update);  // 强制立刻更新
}

void PWM_SetFreq_2(uint16_t freq){
	uint16_t psc = 720000/freq - 1;
	TIM_PrescalerConfig(TIM3, psc, TIM_PSCReloadMode_Update);
	TIM_GenerateEvent(TIM3, TIM_EventSource_Update);  // 强制立刻更新
}
